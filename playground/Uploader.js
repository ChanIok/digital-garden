...
export class Uploader {
  ...
  async uploadMissingFiles(latestManifest) {
    ...
    const filteredPaths = this.filePaths.filter(
      (item) => item !== "manifest.json"
    );

    // Upload files in promises
    const promises = filteredPaths.map((item) =>
      this.uploadFile(item, hashToPath)
    );
    ...
    const finalManifestUploadResponse = await this.bundlr.upload(
      finalManifestContent,
      { tags: this.tags }
    );
